...
CONVERSATIONS = {
    ...
    # 长对话（12 轮）：前 5 组都只有 4 轮，覆盖不到长历史下的行为。
    # 本轮串联：多候选澄清 → **回复序号** → 指代承接 → 话题切换 → 再次澄清
    # → 序号 → 场景切换（售后）→ 反复 → 情绪升级 → 投诉转人工。
    # ⚠️ 序号回复（T3/T7）此前从未测过：`ref_candidates` 不跨轮保留，
    #    需靠消解器从历史里的澄清消息（带 1️⃣2️⃣）自行解析序号。
    "对话6·长对话（12 轮·串联）": [
        "你们有智能门锁吗",
        "多少钱",                      # → 多候选澄清
        "第2个",                       # 序号回复：应解析出具体型号
