...
4. 一条消息含多个意图时（如"这灯多少钱？另外怎么退货？"），取主导/最紧急意图，
   优先级：risk > complaint > aftersale > presale > general > image；
   次要意图在 logic 中简述（如"主意图退货咨询，次要意图售前价格"）

仅输出符合 schema 的 JSON（type / sub_scenario / risk / logic 四个字段）。
"""
