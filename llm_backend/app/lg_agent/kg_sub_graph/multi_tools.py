...
def create_rerank_node(
    ...
    """
    创建重排节点：对 Cypher 查询返回的记录做 Cross-Encoder 精排。

    插入位置：text2cypher/predefined_cypher -> rerank -> summarize

    工作方式：
        1. 从 state 中取出所有 Cypher 查询返回的 records
        2. 用 BGE-Reranker 对每条 record 打分
        3. 按分数降序排列，只保留 top-K 条
        4. 写回 state 供下游 summarize 使用
    ...
    async def rerank(state: OverallState) -> Dict[str, Any]:
        question = state.get("question", "")
        cyphers = state.get("cyphers", list())

        all_records = []
        for cypher in cyphers:
            records = cypher.get("records", list())
            if records:
                all_records.extend(records)

        if not all_records:
            logger.info("无记录需要重排，跳过")
            return {"steps": ["rerank"]}

        # 用 BGE-Reranker 重排
        reranked = reranker.rerank_cypher_records(
            query=question,
            records=all_records,
            top_k=top_k,
        )

        # 将重排后的记录写回第一个 cypher 的 records（保持 state 结构不变）
        updated_cyphers = []
        if cyphers:
            first = dict(cyphers[0])
            first["records"] = reranked
            updated_cyphers.append(first)

        return {"cyphers": updated_cyphers, "steps": ["rerank"]}
...
def create_multi_tool_workflow(
    ...
    rerank = create_rerank_node(top_k=DEFAULT_TOP_K)
    ...
    main_graph_builder.add_node(rerank)
    ...
    main_graph_builder.add_edge("error_tool_selection", "rerank")
    main_graph_builder.add_edge("text2cypher", "rerank")
    main_graph_builder.add_edge("predefined_cypher", "rerank")
    main_graph_builder.add_edge("rerank", "summarize")
