...
9. **多候选检测（重要）**：如果用户当前问题用了指代词或省略，而上文中有**多个同等合理**
   ...
   **此时不要擅自选定一个**，把这些候选放进 candidates；其余情况 candidates 必须为空数组。
   · **candidates 非空 ⟺ 歧义**：凡 candidates 里有值，resolved 就必须原样返回用户消息；
     凡正常消解（resolved 已补全对象），candidates 就必须为空数组——两者不可同时成立
   · **范围缩到恰好 1 个才正常消解**。上文只有一个可指代对象时，把它补进 resolved、
     candidates 留空（实测教训：曾出现"resolved 未消解 + candidates 只有 1 个"的自相矛盾
     输出，导致残缺 query 进入检索、答非所问）
   · **但"缩窄后仍有 ≥2 个"依然是歧义**——这一点不要被上一条冲淡。例：助手列了两款小米门锁，
     用户说"小米那款保修多久"，范围缩到那两款后仍无法唯一确定 → candidates 列出**该两款**、
     resolved 原样返回。（实测教训：收紧"单候选不算歧义"时曾把这条一并放过，
     模型直接选定一款，等于退回"擅自选定"的老毛病）
