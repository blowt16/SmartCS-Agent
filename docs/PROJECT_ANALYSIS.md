...
**模块职责**：被识别节点与各 LLM 业务节点调用（`MemoryManager(llm, cache=MemoryCache())` 每次调用新建），把完整 `state.messages` 压缩到 token 阈值内：最近 N 轮原文 + 老消息两级摘要。
...
flowchart TD
    ...
    F --> G
    G --> H["阈值裁剪：摘要超 800 tokens 按句裁剪<br/>最近对话超 2000 tokens 丢最老的"]
...
> 只设"各部分裁剪阈值"，无全局总量校验：总量取决于模型上下文窗口，由调用方按实际模型决定（当前 `deepseek-v4-flash` 窗口远大于早期设计所设想的 8K）。2026-09-18 随 `TokenBudgetManager` 精简移除 `system_prompt`/`documents`/`response` 三项从未生效的额度与 `total_budget`（`documents_text` 无调用方传入，另两项计数后未被读取）。
...
- Token 阈值裁剪：摘要按句截断、最近对话丢最老的（只管各部分阈值，不做总窗口兜底）
