...
### 3.2 prompt 强化（`RESOLVE_SYSTEM_PROMPT` 重写）

现有 prompt 已覆盖规则 1（代词替换）/规则 2（省略补全）/规则 3（自包含返回）/规则 4（不添加）/规则 5（仅输出文本）——**结构可用，表述强化**：

1. **规则 1/2 合并强化为统一指令**："根据对话历史，把当前问题中依赖上下文的成分（指代词、省略的主语/宾语、不完整信息）补全为完整、独立的问题"
2. **规则 3 升级为显式主路径**（主流出口）："如果当前问题已完整独立（包含明确主语、不依赖上下文），**直接原样返回，不要添加或修改任何信息**"
3. **新增指令类规则**（Nodrat 实践）："如果当前问题是命令式指令（如'查一下价格'），补全为完整的查询意图，不要改写成实体搜索"
4. 保留：规则 4（不添加历史未出现信息）、规则 5（仅输出问题文本）——输出截断（`RESOLVE_MAX_TOKENS=200`）不变
