...
> **归档状态**: ✅ 已完成（2026-09-06 归档 `已完成/`）——落地证据：P0 docx H3 (SKU:) 标注+重建（f230a91）、C sku_codes 注入链（f230a91）、D 返回透出与双通道渲染 + D-2 sku 精确参数（8de0baa）、验证期修复（8a2abd5：none_as_null）。验收全过（PG docker 环境实测）：A1（H3 50/50 锚点=TSV）、A2~A6（parse 段级 sku 全覆盖/重灌 sku_codes 覆盖 50/50/政策 4 块 SQL NULL/多商品块占比 92.1% 与 DB-free 模拟吻合/最长 chapter 95<255）、A7~A10（检索 doc dict 键/工具前缀行/product_stock_lookup sku 精确命中-miss-empty/名称通道回归）、E2E（块编码→动态行同源一致）。单测 78 passed（不含 BM25 隔离用例——已知缺陷见下）；**归档前置验证项填数**：多商品块占比 **92.1%（35/38）** 佐证多值收集为硬需求
