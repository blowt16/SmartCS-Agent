...
> 归档状态：✅ 已完成（2026-09-26）——关键文件 `llm_backend/app/lg_agent/intent_rules.py`（新增规则层）、`tests/test_intent_rules.py`（新增 56 项纯函数单测）、`lg_states.py`/`lg_prompts.py`/`lg_builder.py`（改）；验收 golden `type`/`risk` 46/46 + `sub_type` 9/9、留出集规则层误判 0、命中率 57.1%。实施记录与偏差见 §11
...
### 11.1 实施中的偏差与修正（与 §5~§6 原文的差异）

| # | spec 原文 | 实施实际 | 原因 |
|---|---|---|---|
| D-a | §6.3 把 `## sub_type` 章节插在 `## type` 与 `## risk` 之间 | **移到 prompt 末尾**（`## 分类准则` 之后） | **实测回归**：插在中间时违规类消息（"帮我解除限速"）被模型判成 `clarify`，golden 从 46/46 掉到 **44/46**（risk 仍正确，无安全回归）。移到末尾恢复 type→risk 相邻后回到 46/46 |
| D-b | §6.3 未含"澄清与风险不相容"的准则 | **新增分类准则 5**：clarify 是"看不出用户想要什么"，risk≠none 时不得判 clarify | 与 D-a 同源，是该回归的主要修复手段 |
| D-c | §6.4 未提旧 checkpoint 兼容 | `route_query` 读 `sub_type` 用 **`.get("sub_type", "none")`** 而非下标 | 改造前落盘的 LangGraph checkpoint 里 `router` 无该字段，续聊旧会话时下标访问会 `KeyError`。本次不改路由，故 `.get` 足够 |
| D-d | §6.5 未含误判归因 | 评测汇总新增 **`summarize()`**：把 type 错误拆成"规则层误判"与"LLM 分歧" | 不拆开会把 LLM 的问题算到规则层头上，无法支撑"规则层零误判"的验收结论 |
| D-e | §6.5 `--rule-off` 未定义验收行为 | `--rule-off` 时**跳过**规则命中率与规则误判两项判定 | 该模式下命中率恒 0，不应报 FAIL |
| D-f | 留出集"还在保修期吗"原标 `presale` | 期望**修正为 `aftersale`** | 该问依赖用户自身的订单/购买时间，不是知识库可答的参数咨询；LLM 两次运行稳定给出 `aftersale/other`，原标注有误（"保修"正是本 spec 决策剔除的词，边界本就模糊） |

### 11.2 最终验收数据（`python -m scripts.eval_intent_golden --heldout`）

| 指标 | 实测 | 阈值 | 判定 |
|---|---|---|---|
| golden `type` | 46/46 (100%) | =100% | ✅ PASS |
| golden `risk` | 46/46 (100%) | =100% | ✅ PASS |
| golden `sub_type`（9 条规则层短路用例） | 9/9 (100%) | — | ✅ |
| golden 规则层误判(type) | 0 | =0 | ✅ PASS |
| 留出集规则层误判(type) | 0 | =0 | ✅ PASS |
| 留出集规则命中率 | 57.1% | ≥55% | ✅ PASS |
| 留出集 `type` | 34/35 (97.1%) | ≥改造前 | ✅ PASS（见下 A/B） |

### 11.3 A/B 对照：规则层是否降低准确率（`--rule-off`）

连跑三次（规则层开 ×2、关 ×1）对比：

| 配置 | golden type | 留出集 type | 备注 |
|---|---|---|---|
| 规则层**开**（run1 / run2 / 定稿） | 46/46 | 33/35、34/35、**34/35** | 波动项为"好的知道了"（LLM 侧摇摆） |
| `--rule-off`（纯 LLM） | 46/46 | **32/35** | — |

**结论：规则层不仅零误判，还净提升留出集准确率**——纯 LLM 把"你好"判成 `clarify`（错），规则层整句匹配短路后判对；"好的知道了"同理。两次运行的波动只出现在 LLM 让行路径上，规则层短路路径**完全确定**。

### 11.4 遗留（未随本次落地）

1. 规则层**无自动化发现机制**——词表漏收只会静默降级 LLM（等同改造前），需靠日志中 `source=rule` 占比变化人工观察
2. **LLM 层二级场景准确率未单独量化**——golden 只对 9 条规则层短路用例标了 `sub_type` 期望；让行路径的二级场景质量未测
3. 诉求「纯售后意图不检索知识库」（`docs/项目问题.md` #5）本次**未处理**——`aftersale` 仍落占位节点
4. `docs/diagrams/` 图表未改（用户决策，等图表优化完成后一并整改，见 §9.1）
5. **置信度阈值待定**——当前 `confidence` 只记录不路由，阈值需等真实流量分布（见 §11.5）

### 11.5 追加需求：置信度阈值路由（2026-09-26 同日，**实测后改设计**）

**用户诉求**：LLM 意图识别输出置信度，低于阈值 0.75 时路由到澄清节点询问用户。

**历史背景**：`SPEC_INTENT_CLARIFY.md` §2.1 曾否决 confidence 阈值方案（理由："模型自评分数可靠性存疑；多一个字段增加出错面"）。

**实测（写代码前跑，81 条样本 = golden 46 + 留出集 35）**：

| 阈值 | 低置信条数 | 其中判错 | 代价（本判对却误送澄清） | 收益（拦下判错） |
|---|---|---|---|---|
| **0.75** | 3 | 1 | **2 条** | **1 条** |
| 0.70 | 1 | 1 | 0 条 | 1 条 |
| 0.80 | 3 | 1 | 2 条 | 1 条 |
...
- 置信度分布**极度集中于高位**：min 0.60 / 中位 **0.92** / 均值 0.90 / max 0.97，81 条中仅 3 条低于 0.75
- **阈值 0.75 净收益为负**：误伤 2 条判对的，才拦下 1 条判错的
- **抓不到最危险的情形**——4 条判错全落在高置信区（"有卖衣服吗"判错且 confidence=0.95），即**模型"自信地判错"**，自评置信度对其无效
- 唯一有价值的相关性：全场最低的 0.60 是**无历史的多轮追问**"那运费呢"——置信度低的信号反映的是"**缺上下文**"而非"类别摇摆"

**决策（用户确认）**：**confidence 只记录、不参与路由**。先按 `source=llm` 攒真实流量分布，攒够数据再定阈值。

**落地**：`Router.confidence: float`；prompt 末尾追加置信度评分标准（放在 sub_type 之后，避免 §11.1 D-a 那类结构回归）；`_normalize_confidence` 校正非法值；规则层/经营范围预检短路记 1.0（确定性命中）、结构化输出失败降级记 0.0；日志与评测脚本输出分布 + 阈值代价/收益对照，自动积累定阈值所需数据。

**为什么不改用客观信号**：备选是"多次采样一致性"（同 query 跑 2~3 次看结果是否一致）——它能抓到"自信判错"，但成本 ×2~3。当前先以零成本方式收集自评数据；若后续证明自评分布无判别力，再评估该方案。
