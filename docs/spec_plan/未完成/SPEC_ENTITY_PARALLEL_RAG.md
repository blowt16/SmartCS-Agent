...
2. 不拆场景（单任务 = 原 query 逐字）：单实体的多个属性细节问、条件型选购/推荐句（仅品类、无具体型号对比）、单一意图问；**同实体属性细节禁止拆**
...
3. 其余情况一律不拆：
   ...
   包括：同一商品的多个属性细节问（功能+噪音+续航，必须合并）、
        条件型选购/推荐句（仅提到品类、无具体型号对比）、单一意图问。
...
示例（商品名取自知识库真实在售：米家智能晾衣机/小米智能门锁/智能电动沙发等）：
- 问题：米家智能晾衣机2的承重是多少公斤？
  entity_count：1
  任务：["米家智能晾衣机2的承重是多少公斤？"]
  # 单一意图问 → 不拆，原句
...
- 问题：米家智能晾衣机2和米家智能晾衣机Pro哪个更实用？它们分别多少钱？
  entity_count：2
  任务：["米家智能晾衣机2的功能实用性、优缺点和售价是多少？",
        "米家智能晾衣机Pro的功能实用性、优缺点和售价是多少？"]
  # 多实体 → 每实体 1 条，继承对比+价格意图；"它们分别多少钱"的指代被实体全名替换

- 问题：米家智能晾衣机有哪些型号？分别多少钱？
  entity_count：1（"米家智能晾衣机"为系列名，实体按品类"智能晾衣机"计 1）
  任务：["米家智能晾衣机有哪些型号？",
        "米家智能晾衣机各型号的售价分别是多少？"]
  # 列表型 + 详情型并列 → 拆 2；此时任务数 > entity_count 是合法的（系列/品类主题计 1 个实体）

- 问题：小米智能门锁M30支持人脸识别吗？续航多久？有远程告警吗？
  entity_count：1
  任务：["小米智能门锁M30支持人脸识别吗？续航多久？有远程告警吗？"]
  # 同一商品的属性细节问 → 合并为 1 条原句，不拆

- 问题：有没有适合小户型的智能电动沙发？
  entity_count：1（"智能电动沙发"为品类实体，无具体型号）
  任务：["有没有适合小户型的智能电动沙发？"]
  # 条件型选购句（仅品类、无型号对比）→ 不拆，原句
