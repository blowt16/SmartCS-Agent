...
- 目标：阶段 2 按 §4.2 重写——替换用**完整 prompt 全文（2026-09-03 v3 精简稿，5 示例）**已在 §4.2 落档：子问单元拆解模型（多实体对比拆、列表+详情并列拆两类动机；同实体属性细节/仅品类选购/单意图不拆）、单任务强制=原 query、MAX_TASKS=3，schema 与节点校验同步更新（数量锚废除、空列表由节点 `_fallback` 兜底）。
...
>
> **2026-09-03 v3 精简（用户评审后）**：仅行文精简（正文约 -45%、示例约 -30%），**决策模型/字段/校验零改动**——删除与节点 `_fallback` 兜底重复的"空列表禁令"、规则间重复的指代/自含主题词表述与引导冗余；示例压缩为单行式 `entity_count：N → 任务：[…]`（few-shot 密度不变）。**全文以 v3 为准**。
...
**提示词规则**（重写 `kg_sub_graph/prompts/kg_prompts.py` 的 `PLANNER_SYSTEM_PROMPT`；2026-09-03 v3 精简稿）：

1. 拆分动机只有两种，其余一律不拆：**多实体（≥2）** → 每实体 1 任务、继承原 query 对该实体的全部意图；**列表型（有哪些/几款/推荐）+ 详情型（价格/参数/功能）并列** → 拆 2
2. 不拆场景 = 单任务原文（禁止改写）：同商品多条属性细节问（功能+噪音+续航）、仅品类的选购句、单意图问；**拿不准也合并为 1 条**
3. 通用约束：任务文本自含实体全名/品类词、可独立检索、禁止指代（它/它们/这款）；任务间不重复、不依赖；**总数 ≤ 3**
4. `entity_count`：句中明确提到的商品/品类实体数（泛指词不算，无实体则 0）——决策辅助与日志，非数量校验锚

**替换用完整 prompt 全文（阶段 2 Step 2 产物，2026-09-03 v3 精简稿——正文约 -45%、示例约 -30%，决策模型/字段/校验零改动）**：
...
示例（商品名取自知识库真实在售）：
- 问题：米家智能晾衣机2的承重是多少公斤？
  entity_count：1 → 任务：["米家智能晾衣机2的承重是多少公斤？"]     # 单意图不拆

- 问题：米家智能晾衣机2和米家智能晾衣机Pro哪个更实用？它们分别多少钱？
  entity_count：2 → 任务：["米家智能晾衣机2的功能实用性、优缺点和售价是多少？",
                          "米家智能晾衣机Pro的功能实用性、优缺点和售价是多少？"]
  # 多实体每实体 1 条；指代"它们"被实体全名替换

- 问题：米家智能晾衣机有哪些型号？分别多少钱？
  entity_count：1 → 任务：["米家智能晾衣机有哪些型号？",
                          "米家智能晾衣机各型号的售价分别是多少？"]
  # 列表+详情并列拆 2（任务数可大于 entity_count）

- 问题：小米智能门锁M30支持人脸识别吗？续航多久？有远程告警吗？
  entity_count：1 → 任务：["小米智能门锁M30支持人脸识别吗？续航多久？有远程告警吗？"]  # 属性细节问合并不拆

- 问题：有没有适合小户型的智能电动沙发？
  entity_count：1 → 任务：["有没有适合小户型的智能电动沙发？"]  # 仅品类的选购句不拆
...
**为什么在消解之后**：`"那个有货吗"` 与 `"米家智能晾衣机2有货吗"` 语义相同但字面不同，若不先消解，两者成为不同缓存 key，命中率大降。
...
> - P1：`PLANNER_SYSTEM_PROMPT` 整体替换为 §4.2 v3 精简稿全文（2026-09-03；5 条真实商品示例覆盖：多实体对比拆、列表+详情并列拆、同实体多属性合并、单意图不拆、仅品类选购句不拆）
...
- [ ] **Step 2**: 按 §4.2「替换用完整 prompt 全文（2026-09-03 v3 精简稿）」重写：`PLANNER_SYSTEM_PROMPT`（kg_prompts.py）整体替换 + human 模板（planner/prompts.py）缩减为仅 `问题: {question}`（删除其中与 system 重复的 4 条规则）
...
- [ ] **Step 4**: 验证（断言均为 v2 日志口径 `reason=tasks=k`）：
  - "米家智能晾衣机2和米家智能晾衣机Pro哪个更实用？它们分别多少钱？"（多实体）→ `split=2`，两条 Sub Task 各含实体全名与价格意图，无"它们"指代
  - "米家智能晾衣机有哪些型号？分别多少钱？"（列表+详情并列，entity_count=1）→ `split=2`（合法：任务数 > entity_count）
  - "小米智能门锁M30支持人脸识别吗？续航多久？有远程告警吗？"（同实体多属性）→ `fallback (reason=tasks=1)`，任务文本 = 原 query 原文
  - "有没有适合小户型的智能电动沙发？"（条件型选购，仅品类）→ `fallback (reason=tasks=1)`，任务 = 原 query 原文（非 LLM 改写）
...
|---|---|---|---|
| 单实体简单查询首轮（"米家智能晾衣机2的承重是多少"） | 3（Router+planner+summarize） | ≤3（P4 规则直通若落地可 ≤2） | 日志计数 3；单分支检索 |
| 双实体对比（"米家智能晾衣机2和Pro哪个更实用"） | 3（拆 N 分支不增 LLM，检索零调用） | ≤5（阶段 4 HyDE 每分支 +1，2 分支共 +2） | planner 输出 entity_count≥2 且 N 个并行分支日志 `检索节点返回…` ×N |
...
| `kg_sub_graph/prompts/kg_prompts.py` | `PLANNER_SYSTEM_PROMPT` 整体替换为 §4.2 v3 精简稿全文（5 真实商品示例；现状仍为北风商贸 Cypher 时代模板，§2.3 P1） | 2⏳ |
