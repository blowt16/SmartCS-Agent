...
| 项 | 实测值 |
|---|---|
| 规则层命中率（真实问句留出集，31 条） | 55% → 这部分 query **省一次 LLM 调用** |
| 规则层命中率（现有 golden set，46 条难例集） | 37% |
| 命中后场景判错（两个集合合计 77 条） | **0 条** |
...
| # | 决策点 | 结论 |
|---|---|---|
| D1 | 顶层 `complaint` 与二级场景 `complaint` 冲突 | **顶层保留，二级去掉 complaint**。凡投诉语义一律走顶层 complaint 节点，二级仅剩 5 类 |
...
| `logistics` | `logistics_query` | 改名（对齐用户口径） |
| `order_query` | — | **删除**，"查订单/订单状态"归入 `logistics_query`（口径见 §5.3） |
| — | `exchange` | 新增（换货） |
| — | `reship` | 新增（补发） |
| — | `other` | 新增（售后兜底） |
...
| 留出集（手写 31 条真实问句，不在 golden 内） | 31 | 17（55%） | **0** |

留出集未命中的 14 条，全部是**有意不命中**：clarify 3 条、complaint 3 条（不在 D3 覆盖范围）、风险信号 3 条（D2 闸门）、多意图 1 条、词表未覆盖的 presale 4 条。
...
> 词表只收高辨识度业务词，泛词（怎么 / 什么 / 怎么样 / 支持 / 功能 / 保修 / 质保）一律不收。
...
#### 需用户确认的口径：`order_query` 的归宿

旧维度有 `order_query`（订单查询），本次删除（§2.3）。"查一下我的订单""我的订单状态是什么"归 **`logistics_query`**——理由是订单状态绝大多数即物流状态。若认为两者语义应分开，则这两句应归 `other` 或恢复 `order_query` 类目，**请在本 spec 审核时指明**。
...
> ⚠️ **待确认项**：`保修` / `质保` 存在语义歧义——"这个锁保修多久"（知识库可答，presale）vs "我要保修"（售后诉求）。当前词表收进 presale，会把后者也判成 presale。**备选**：把这两个词移出词表（交回 LLM 决定，等价改造前行为，最保守）。
> 实测依据：现有 golden set 与留出集均无用例依赖这两个词，移出不影响任何已测指标。
...
- logistics_query 物流查询：问包裹/快递/发货/配送/签收/订单状态
  （如"我的快递到哪了""什么时候发货""查一下我的订单"）
...
2. "查订单/订单状态"归 logistics_query（订单状态通常即物流状态）
...
2. **新增留出集 `HELDOUT`**：把 §3.2 验证用的 31 条真实问句固化进脚本（防止词表在迭代中不知不觉过拟合 golden set）
...
| 2 | "这个锁保修多久" | 命中 `保修` | `presale/none`（**规则**） | ⚠️ 见 §5.4 待确认项 |
...
| 规则层命中率（留出集） | ≥ 50% | §3.2 实测 55%，实施后不得显著低于 |
...
`docs/diagrams/` 下有 `00a/00b/10/11/20` 五张流程图，`10-presales-orchestration` 与 `00b-dialogue-dispatch` 的 spec JSON 含意图路由节点。本次改 `Router` 字段与判定层次，**是否需要重出这两张图**请审核时指明（另有 `30-intent-routing` 图在 `docs/superpowers/plans/2026-09-26-系统运行流程图.md:249-262` 中规划但尚未产出——若本次要做，正好一并产出）。
...
| # | 项 | 说明 |
|---|---|---|
| R1 | **规则误判是新增故障面** | 改造前所有 query 都过 LLM，判错概率由 LLM 决定；改造后 37~55% 的 query 由词表决定。缓解：判据"宁可漏不可错"+ 留出集零误判 + 单测覆盖边界 |
